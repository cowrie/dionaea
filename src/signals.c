...
void sighup_cb(struct ev_loop *loop, struct ev_signal *w, int revents)
{
  ...
  /* ToDo
	...
	{
		g_critical("lcfg error: %s", lcfg_error_get(g_dionaea->config.config));
	}
...
#define BACKTRACE_SIZE 32
	...
	for( GList *it = g_dionaea->logging->loggers; it != NULL; it = it->next )
	{
		...
		const char *msg =
			...
			"You can create better backtraces with gdb, for more information visit https://github.com/cowrie/dionaea\n"
