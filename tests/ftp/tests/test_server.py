...
class TestDispatch:
    ...
    def test_incident_fired_for_commands(self):
        ...
        d.handle_established()
        feed(d, 'NOOP')
        ftp_incidents = [i for i in MockIncident._all_incidents
...
class TestLogin:
    ...
    def test_login_incident(self):
        ...
        feed(d, 'USER testuser')
        feed(d, 'PASS secret')
        login_incidents = [i for i in MockIncident._all_incidents
...
class TestPreAuth:
    def test_feat_before_login(self):
        ...
        d._sent.clear()
        feed(d, 'FEAT')
        text = ''.join(b.decode() if isinstance(b, bytes) else b
                       for b in d._sent)
        assert '211' in text
