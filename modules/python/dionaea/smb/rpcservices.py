...
def _get_pointer_size(con):
    """Get negotiated pointer size from connection state."""
    return con.state.get("pointer_size", 32)


def make_unpacker(con, data):
    """Create an Unpacker with the correct pointer size for this connection."""
    return ndrlib.Unpacker(data, pointer_size=_get_pointer_size(con))


def make_packer(con):
    """Create a Packer with the correct pointer size for this connection."""
    return ndrlib.Packer(pointer_size=_get_pointer_size(con))
...
class RPCService:
    ...
    def processrequest(cls, service, con, opnum, p):
        if opnum in cls.ops:
            ...
            if method is None:
                rpclog.warning(
                    "Unsupported DCERPC operation: %s %s (0x%02x) - no handler",
                ...
                if opnum in cls.vulns:
                    ...
                    rpclog.warning(
                        "Potential %s exploit: %s %s (0x%02x)",
                        vulnname,
                        service.__class__.__name__,
                        opname,
                        opnum,
                ...
                try:
                    ...
                    rpclog.warning(f"Exploit attempt detected: {e}")
                    return None
                except EOFError:
                    rpclog.warning(
            ...
            rpclog.warning(
                "Unknown DCERPC operation: %s opnum 0x%02x",
                service.__class__.__name__,
                opnum,
            )
...
class DSSETUP(RPCService):
    ...
    ops = {0x09: "DsRolerUpgradeDownlevelServer"}
    vulns = {
        0x09: "MS04-11",
    }
...
class epmp(RPCService):
    ...
    # ept_lookup inquiry_type values
    RPC_C_EP_ALL_ELTS = 0x00  # Return all elements
    RPC_C_EP_MATCH_BY_IF = 0x01  # Match by interface UUID
    RPC_C_EP_MATCH_BY_OBJ = 0x02  # Match by object UUID
    RPC_C_EP_MATCH_BY_BOTH = 0x03  # Match by both
    ...
    def handle_ept_lookup(cls, con, p):
        ...
        # Parse input parameters for logging
        try:
            x = make_unpacker(con, p.StubData)
            inquiry_type = x.unpack_long()
            inquiry_name = cls.INQUIRY_TYPE_NAMES.get(
                inquiry_type, f"UNKNOWN(0x{inquiry_type:x})"
            )
            ...
            object_uuid = None
            if object_ptr:
                object_uuid = str(UUID(bytes_le=x.unpack_raw(16)))

            # Ifid (RPC_IF_ID) pointer - contains UUID + version
            ifid_ptr = x.unpack_pointer()
            ifid_uuid = None
            ifid_version = None
            if ifid_ptr:
                ifid_uuid = str(UUID(bytes_le=x.unpack_raw(16)))
                ifid_major = x.unpack_short()
                ifid_minor = x.unpack_short()
                ifid_version = f"{ifid_major}.{ifid_minor}"

            _ = x.unpack_long()  # vers_option - skip for now

            # Log what they're looking for
            log_parts = [f"inquiry={inquiry_name}"]
            if object_uuid:
                log_parts.append(f"object={object_uuid}")
            if ifid_uuid:
                log_parts.append(f"interface={ifid_uuid} v{ifid_version}")

            rpclog.info(
                "ept_lookup from %s:%d - %s",
                con.remote.host,
                con.remote.port,
                ", ".join(log_parts),
            )
        except Exception as e:
            rpclog.debug("Failed to parse ept_lookup params: %s", e)
...
class lsarpc(RPCService):
    ...
    class LSAPR_OBJECT_ATTRIBUTES:
        ...
        def __init__(self, p):
            ...
            if isinstance(self.__packer, ndrlib.Packer):
                ...
                rpclog.debug(f"Length = {self.Length}")
                self.RootDirectory = self.__packer.unpack_short()
                rpclog.debug(f"RootDirectory = {self.RootDirectory:x}")
                self.ObjectName = self.__packer.unpack_pointer()
                rpclog.debug(f"ObjectName = {self.ObjectName:x}")
    ...
    class LSAPR_TRANSLATED_SIDS:
        ...
        def __init__(self, p):
            ...
            if isinstance(self.__packer, ndrlib.Packer):
                ...
                rpclog.debug(f"Entries = {self.Entries}")
    ...
    ops = {0: "Close", 44: "OpenPolicy", 57: "LookupSids2", 58: "LookupNames2"}
    ...
    def handle_OpenPolicy(cls, con, p):
        ...
        if system_ptr != 0:
            ...
            SystemName = b""
        rpclog.debug(f"ServerName {SystemName}")
    ...
    def handle_LookupSids2(cls, con, p):
        ...
        rpclog.debug(f"EntriesRead = {SidEnumBuffer.Entries}")
        ...
        rpclog.debug(f"LookupLevel {LookupLevel} MappedCount {MappedCount} LookupOptions {LookupOptions} ClientRevision {ClientRevision}")
    ...
    def handle_Close(cls, con, p):
        ...
        rpclog.debug(f"ObjectHandle {ObjectHandle}")
...
class NWWKS(RPCService):
    ...
    ops = {0x09: "NwOpenEnumNdsSubTrees", 0x01: "NwChangePassword"}
    vulns = {
        0x09: "MS06-66",
        0x01: "MS06-66",
    }
...
class MGMT(RPCService):
    ...
    class rpc_if_id_t:
        ...
        def show(self):
            rpclog.debug(
                f"uuid {self.uuid} {self.vers_major}.{self.vers_minor}"
            )
...
class samr(RPCService):
    ...
    class RPC_UNICODE_STRING:
        ...
        def __init__(self, p, c=1):
            ...
            if isinstance(self.__packer, ndrlib.Packer):
                ...
                rpclog.debug(f"Count = {self.Count}")
    ...
    def handle_Connect4(cls, con, p):
        ...
        if server_ptr != 0:
            ...
            ServerName = b""
        rpclog.debug(f"ServerName {ServerName}")
        DesiredAccess = x.unpack_long()
        rpclog.debug(f"DesiredAccess {DesiredAccess}")
        ClientRevision = x.unpack_long()
        rpclog.debug(f"InVersion {ClientRevision}")
    ...
    def handle_Connect5(cls, con, p):
        ...
        rpclog.debug(f"InVersion {InVersion}")
        ...
        rpclog.debug(f"Revision {Revision} SupportedFeatures {SupportedFeatures}")
    ...
    def handle_QuerySecurityObject(cls, con, p):
        ...
        rpclog.debug(f"SecurityInformation {SecurityInformation}")
    ...
    def handle_EnumDomains(cls, con, p):
        ...
        rpclog.debug(f"ServerHandle {ServerHandle}")

        EnumerationContext = x.unpack_long()
        rpclog.debug(f"EnumerationContext {EnumerationContext}")

        PreferedMaximumLength = x.unpack_long()
        rpclog.debug(f"PreferedMaximumLength {PreferedMaximumLength}")
    ...
    def handle_EnumDomainUsers(cls, con, p):
        ...
        rpclog.debug(f"DomainHandle {DomainHandle}")

        EnumerationContext = x.unpack_long()
        rpclog.debug(f"EnumerationContext {EnumerationContext}")

        UserAccountControl = x.unpack_long()
        rpclog.debug(f"UserAccountControl {UserAccountControl}")
    ...
    def handle_LookupNamesInDomain(cls, con, p):
        ...
        # unsigned long Count
        Count = x.unpack_long()
        rpclog.debug(f"Count {Count}")

        # RPC_UNICODE_STRING Names[*]
        Maxcount = x.unpack_long()
        rpclog.debug(f"Maxcount {Maxcount}")
        Offset = x.unpack_long()
        rpclog.debug(f"Offset {Offset}")
        ActualCount = x.unpack_long()
        rpclog.debug(f"ActualCount {ActualCount}")
        Names = samr.RPC_UNICODE_STRING(x, Count)

        global LookupName
        LookupName = Names.Buffer.decode("UTF-16")
        rpclog.debug(f"LookupName {LookupName}")
    ...
    def handle_QueryInformationUser(cls, con, p):
        ...
        rpclog.debug(f"UserHandle {UserHandle}")

        UserInformationClass = x.unpack_short()
        rpclog.debug(f"UserInformationClass {UserInformationClass}")
        ...
        if UserInformationClass == 21:
            ...
            rpclog.debug(f"LookupName {cls.LookupName}")
    ...
    def handle_QueryDisplayInformation(cls, con, p):
        ...
        rpclog.debug(f"DisplayInformationClass {DisplayInformationClass}")

        Index = x.unpack_long()
        rpclog.debug(f"Index {Index}")

        EntryCount = x.unpack_long()
        rpclog.debug(f"EntryCount {EntryCount}")

        PreferredMaximumLength = x.unpack_long()
        rpclog.debug(f"PreferredMaximumLength {PreferredMaximumLength}")
    ...
    def handle_Close(cls, con, p):
        ...
        rpclog.debug(f"SamHandle {SamHandle}")
...
class spoolss(RPCService):
    ...
    class PRINTER_INFO_1:
        ...
        def size(self):
            ...
            rpclog.debug(f"rpclog.debugER_INFO_1 size {size}")
            return size
    ...
    def handle_EnumPrinters(cls, con, p):
        ...
        rpclog.debug(f"Flags {Flags} Name {Name} Level {Level} cbBuf {cbBuf} ")
        ...
        a.Buffer = [
            "Internet URL Printers\0",
            "Windows NT Internet Provider\0",
            "Windows NT Internet Printing\0",
            "Remote Printers\0",
            "Windows NT Remote Printers\0",
            "Microsoft Windows Network\0",
            "Locally Connected Printers\0",
            "Windows NT Local Print Providor\0",
    ...
    def handle_StartDocPrinter(cls, con, p):
        ...
        rpclog.debug(f"hPrinter {hPrinter}")

        Level = x.unpack_long()
        rpclog.debug(f"Level {Level}")

        DocInfo = spoolss.DOC_INFO_1(x)
        DocName = DocInfo.DocName.decode("UTF-16")[:-1]
        OutputFile = DocInfo.OutputFile.decode("UTF-16")[:-1]

        rpclog.debug(f"docname {DocName} outputfile {OutputFile}")

        if OutputFile.startswith("\\") and OutputFile.endswith(r"\PIPE\ATSVC"):
    ...
    def handle_WritePrinter(cls, con, p):
        ...
        if p.PacketFlags == 0:
            ...
            con.printer += p.StubData
            x = make_unpacker(con, con.printer)
            x.unpack_raw(20)
            cbBuf = x.unpack_long()
...
class SRVSVC(RPCService):
        ...
        def pack(self):
            if isinstance(self.__packer, ndrlib.Packer):
                ...
                for j in self.Data:
                    ...
                    self.__packer.pack_string_fix("\0".encode("utf16")[2:])
    ...
    def handle_NetShareEnum(cls, con, p):
        ...
        rpclog.debug(f"infostruct_share {infostruct_share} preferdmaxlen {preferdmaxlen}  resumehandleptr {resumehandleptr:x} resumehandle {resumehandle}")
        ...
        if infostruct_share == 0:
            ...
            raise NotImplementedError(
                f"Unsupported infostruct_share: {infostruct_share}"
            )
    ...
    def handle_NetPathCanonicalize(cls, con, p):
        ...
        rpclog.debug(f"ref 0x{ref:x} server_unc {server_unc} path {path} maxbuf {maxbuf} prefix {prefix} pathtype {pathtype} pathflags {pathflags}")
    ...
    def handle_NetPathCompare(cls, con, p):
        ...
        rpclog.debug(f"ref 0x{ref:x} server_unc {server_unc} path1 {path1} path2 {path2} pathtype {pathtype} pathflags {pathflags}")
        r = make_packer(con)
    ...
    def handle_NetShareAdd(cls, con, p):
        ...
        rpclog.debug(f"infostruct_share {infostruct_share} ptr_parm {ptr_parm:x} ParmErr {error}")
    ...
    def handle_NetrShareGetInfo(cls, con, p):
        ...
        Level = p.unpack_long()
        rpclog.debug(f"NetName {NetName} Level {Level}")
    ...
    def handle_NetNameCanonicalize(cls, con, p):
        ...
        Flags = p.unpack_long()
        rpclog.debug(f"ServerName {ServerName} Name {Name} Outbuflen {Outbuflen} Nametype {NameType} Flags {Flags}")
...
class WKSSVC(RPCService):
    ...
    ops = {0x1B: "NetAddAlternateComputerName"}
    vulns = {
        0x1B: "MS03-39",
    }
