...
class SMB_Header(Packet):
    ...
    fields_desc = [
        ...
        FlagsField(
            "Flags2",
            SMB_FLAGS2_KNOWS_LONG_NAMES | SMB_FLAGS2_UNICODE,
            -16,
            SMB_Header_Flags2,
        ),
...
class SMB_Negociate_Protocol_Request_Counts(Packet):
    ...
    fields_desc = [
        ...
        PacketListField(
            "Requests",
            None,
            SMB_Negociate_Protocol_Request_Tail,
            length_from=lambda x: x.ByteCount,
        ),
    ]
...
class SMB_Negociate_Protocol_Response(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrLenField("EncryptionKey", b"", length_from=lambda x: 0),
            lambda x: not x.Capabilities & CAP_EXTENDED_SECURITY,
        ),
        ConditionalField(
            UnicodeNullField("OemDomainName", "WORKGROUP"),
            lambda x: not x.Capabilities & CAP_EXTENDED_SECURITY,
        ),
        # In [MS-SMB].pdf page 49,
        # "ServerName" field needed for case without CAP_EXTENDED_SECURITY
        ConditionalField(
            UnicodeNullField("ServerName", "HOMEUSER-3AF6FE"),
            lambda x: not x.Capabilities & CAP_EXTENDED_SECURITY,
        ),
        ...
        ConditionalField(
            StrLenField(
                "ServerGUID",
                b"\x0b\xff\x65\x38\x54\x7e\x6c\x42\xa4\x3e\x12\xd2\x11\x97\x16\x44",
                length_from=lambda x: 16,
            ),
            lambda x: x.Capabilities & CAP_EXTENDED_SECURITY,
        ),
        ConditionalField(
            StrLenField("SecurityBlob", b"", length_from=lambda x: 0),
            lambda x: x.Capabilities & CAP_EXTENDED_SECURITY,
        ),
    ]
...
class SMB_Sessionsetup_ESEC_AndX_Request(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrLenField(
                "Padding", "\x00", length_from=lambda x: (x.SecurityBlobLength + 1) % 2
            ),
            lambda x: x.underlayer.Flags2 & SMB_FLAGS2_UNICODE,
        ),
...
class SMB_Sessionsetup_ESEC_AndX_Response(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrFixedLenField(
                "Padding", "\x00", length_from=lambda x: (len(x.SecurityBlob) + 1) % 2
            ),
            lambda x: x.underlayer.Flags2 & SMB_FLAGS2_UNICODE,
        ),
        SMBNullField(
...
class SMB_Sessionsetup_AndX_Request2(Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            StrLenField(
                "Padding", "\x00", length_from=lambda x: (x.PasswordLength + 1) % 2
            ),
            lambda x: x.underlayer.Flags2 & SMB_FLAGS2_UNICODE,
        ),
        SMBNullField(
    ...
    def lengthof_Extrabytes(self):
        ...
        bc = (
            bc
            - len(self.Account)
            - len(self.PrimaryDomain)
            - len(self.NativeOS)
            - len(self.NativeLanManager)
        )
        if hasattr(self, "Padding") and self.Padding is not None:
...
class SMB_Sessionsetup_AndX_Response2(Packet):
    ...
    fields_desc = [
        ...
        MultiFieldLenField(
            ...
            None,
            fmt="<H",
            length_of=("Padding", "NativeOS", "NativeLanManager", "PrimaryDomain"),
        ),
        ConditionalField(
            StrFixedLenField("Padding", b"\0", 1),
            lambda x: x.underlayer.Flags2 & SMB_FLAGS2_UNICODE,
        ),
        SMBNullField(
            "NativeOS",
...
class SMB_Treeconnect_AndX_Response_Extended(Packet):
    ...
    fields_desc = [
        ...
        MultiFieldLenField(
            ...
            None,
            fmt="<H",
            length_of=("Service", "Padding", "NativeFileSystem"),
        ),
        StrNullField("Service", "IPC"),
        ConditionalField(
            StrFixedLenField("Padding", b"\0", 2),
            lambda x: x.underlayer.Flags2 & SMB_FLAGS2_UNICODE,
        ),
        StrNullField("NativeFileSystem", ""),
    ]
...
class SMB_Write_AndX_Request(Packet):
    ...
    fields_desc = [
        ...
        LEShortField("ByteCount", 0),
        ConditionalField(
            LEShortField("PipeWriteLen", 0),
            lambda x: x.WriteMode & SMB_WM_MSGSTART and x.WriteMode & SMB_WM_WRITERAW,
        ),
        StrLenField(
            "Padding",
            None,
            length_from=lambda x: x.ByteCount - ((x.DataLenHigh << 16) | x.DataLenLow),
        ),
        StrLenField(
            "Data", b"", length_from=lambda x: ((x.DataLenHigh << 16) | x.DataLenLow)
        ),
    ]
...
class SMB_Trans2_Request(Packet):
    ...
    fields_desc = [
        ...
        FieldListField(
            "Setup",
            0,
            LEShortEnumField("", 0, SMB_Trans2_Commands),
...
class SMB_Trans2_FIND_FIRST2_Response(Packet):
    ...
    fields_desc = [
        ...
        StrFixedLenField(
            "Data",
            b"\x60\x00\x00\x00\x00\x00\x00\x00\x39\xa3\xda\x08\x01\xd6\xd2\x01",
            16,
        ),
        StrFixedLenField(
            "Data1",
            b"\xba\xb0\x6e\x0a\x01\xd6\xd2\x01\x39\xa3\xda\x08\x01\xd6\xd2\x01",
            16,
        ),
        StrFixedLenField(
            "Data2",
            b"\x39\xa3\xda\x08\x01\xd6\xd2\x01\x00\x00\x00\x00\x00\x00\x00\x00",
            16,
        ),
        StrFixedLenField(
            "Data3",
            b"\x00\x00\x00\x00\x00\x00\x00\x00\x10\x00\x00\x00\x02\x00\x00\x00",
            16,
        ),
        StrFixedLenField(
            "Data4",
        ...
        StrFixedLenField(
            "Data6",
            b"\x64\x00\x00\x00\x00\x00\x00\x00\xc2\xaf\xca\x06\xcc\xd5\xd2\x01",
            16,
        ),
        StrFixedLenField(
            "Data7",
            b"\x9d\x76\x46\x90\xcc\xd5\xd2\x01\xc2\xaf\xca\x06\xcc\xd5\xd2\x01",
            16,
        ),
        StrFixedLenField(
            "Data8",
            b"\xc2\xaf\xca\x06\xcc\xd5\xd2\x01\x00\x00\x00\x00\x00\x00\x00\x00",
            16,
        ),
        StrFixedLenField(
            "Data9",
            b"\x00\x00\x00\x00\x00\x00\x00\x00\x10\x00\x00\x00\x03\x00\x00\x00",
            16,
        ),
        StrFixedLenField(
            "Data10",
        ...
        StrFixedLenField(
            "Data11",
            b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x2e\x2e",
            16,
        ),
...
RAP_Opcodes = {RAP_OP_NETSHAREENUM: "NetShareEnum"}
...
bind_bottom_up(
    ...
    Command=lambda x: x == 0x72,
    Flags=lambda x: x & 0x80,
)
bind_bottom_up(
    SMB_Header,
    SMB_Negociate_Protocol_Request_Counts,
    Command=lambda x: x == 0x72,
    Flags=lambda x: not x & 0x80,
)
...
bind_bottom_up(
    ...
    Command=lambda x: x == 0x73,
    Flags=lambda x: not x & 0x80,
    Flags2=lambda x: not x & SMB_FLAGS2_EXT_SEC,
)
bind_bottom_up(
    SMB_Header,
    ...
    Command=lambda x: x == 0x73,
    Flags=lambda x: not x & 0x80,
    Flags2=lambda x: x & SMB_FLAGS2_EXT_SEC,
)
...
bind_bottom_up(
    SMB_Header,
    SMB_Trans_Request,
    Command=lambda x: x == 0x25,
    Flags=lambda x: not x & 0x80,
)
bind_bottom_up(
    SMB_Header,
    SMB_Trans2_Request,
    Command=lambda x: x == 0x32,
    Flags=lambda x: not x & 0x80,
)
bind_bottom_up(
    SMB_Header,
    SMB_Trans2_Secondary_Request,
    Command=lambda x: x == 0x33,
    Flags=lambda x: not x & 0x80,
)
bind_bottom_up(
    SMB_Header,
    SMB_Trans_Secondary_Request,
    Command=lambda x: x == 0x26,
    Flags=lambda x: not x & 0x80,
)
...
bind_bottom_up(
    SMB_Header,
    SMB_Write_Request,
    Command=lambda x: x == SMB_COM_WRITE,
    Flags=lambda x: not x & 0x80,
)
bind_bottom_up(
    SMB_Header,
...
bind_bottom_up(
    SMB_Header,
    SMB_Delete_Request,
    Command=lambda x: x == SMB_COM_DELETE,
    Flags=lambda x: not x & 0x80,
)
