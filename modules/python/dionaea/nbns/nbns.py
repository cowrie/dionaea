...
def analyze_query_threat(
    name: str, suffix: int, qtype: int, opcode: int
) -> tuple[str, str]:
    """
    ...
    if suffix in (NBNS_SUFFIX_DOMAIN_CONTROLLER, NBNS_SUFFIX_DOMAIN_MASTER):
        return (
            "suspicious",
            "Domain controller/master query - potential AD reconnaissance",
        )
    ...
    def __init__(self, data: bytes):
        ...
        if self.ancount > 100 or self.nscount > 100 or self.arcount > 100:
            self.parse_errors.append(
                f"Unusual RR counts: an={self.ancount} ns={self.nscount} ar={self.arcount}"
            )
        ...
        for _ in range(self.qdcount):
            ...
            if name_len != 32:
                self.parse_errors.append(
                    f"Unusual name length: {name_len} (expected 32)"
                )
            ...
            try:
                ...
                logger.debug(
                    "Failed to decode name: %s (raw: %s)", e, encoded_name.hex()
                )
...
class NBNSResponse:
    ...
    def build_positive_name_query_response(
        transaction_id: int, name: str, suffix: int, ip_address: str, ttl: int = 300000
    ) -> bytes:
        """
...
class nbnsHandler(connection):
    ...
    def handle_io_in(self, data: bytes) -> int:
        try:
            ...
            logger.warning(
                "Failed to parse NBNS packet from %s:%s: %s",
                self.remote.host,
                self.remote.port,
                e,
            )
        ...
        if packet.is_response:
            logger.debug(
                "NBNS response from %s:%s (ignoring)",
                self.remote.host,
                self.remote.port,
            )
            return len(data)
        ...
        for name, suffix, qtype, qclass in packet.questions:
            ...
            if is_wpad:
                logger.info(
                    "WPAD query from %s:%s - potential Hot Potato attack!",
                    self.remote.host,
                    self.remote.port,
                )
            else:
                logger.info(
                    "NBNS %s from %s:%s: name=%s<%02X> (%s) type=0x%04X",
                    opcode_name,
                    self.remote.host,
                    self.remote.port,
                    name,
            ...
            if self.respond:
                ...
                if should_respond:
                    logger.info(
                        "Responding to NBNS query for %s with our IP %s",
                        name,
                        self.local.host,
                    )
                    response = NBNSResponse.build_positive_name_query_response(
                        packet.transaction_id, name, suffix, self.local.host
                    )
                    self.send(response)
...
class nbnsd(connection):
    ...
    def handle_io_in(self, data: bytes) -> int:
        ...
        try:
            ...
            logger.warning(
                "Malformed NBNS packet from %s:%s: %s",
                self.remote.host,
                self.remote.port,
                e,
            )
        ...
        if packet.has_anomalies():
            logger.warning(
                "NBNS packet anomalies from %s:%s: %s",
                self.remote.host,
                self.remote.port,
        ...
        for name, suffix, qtype, qclass in packet.questions:
            ...
            if threat_level == "attack":
                logger.warning(
                    "NBNS ATTACK from %s:%s: %s (name=%s<%02X> type=%s)",
                    self.remote.host,
                    self.remote.port,
                    threat_desc,
                    name,
                ...
                logger.info(
                    "NBNS suspicious from %s:%s: %s (name=%s<%02X> type=%s)",
                    self.remote.host,
                    self.remote.port,
                    threat_desc,
                    name,
                ...
                logger.info(
                    "NBNS %s from %s:%s: name=%s<%02X> (%s) type=%s",
                    opcode_name,
                    self.remote.host,
                    self.remote.port,
                    name,
